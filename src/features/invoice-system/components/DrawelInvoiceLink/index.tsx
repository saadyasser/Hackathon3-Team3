...
const DrawelInvoiceLink: any = (): any => {
    ...
    const {
        ...
        error,
        isLoading,
    } = useSwrFetch(API_SERVICES_URLS.CLIENT.INVOICE_LINK_DETAILS("641c418f86abbe326e82bc04"), {
        method: "GET",
        headers: {},
    });
    ...
    const changeStateToInactive = () => {
        axios.put('https://talents-valley-backend.herokuapp.com/api/service/change-status/634e949b6a8f7123704482e8', { status: "inactive" }, { headers: { Authorization: `Bearer ${currentUser.accessToken}` } })
            .then(response => {
    ...
    const changeStateToActive = () => {
        // const currentUser = getCookie(COOKIES_KEYS.currentUser);
        axios.put('https://talents-valley-backend.herokuapp.com/api/service/change-status/634e949b6a8f7123704482e8', { status: "active" }, { headers: { Authorization: `Bearer ${currentUser.accessToken}` } })
            .then(response => {
                console.log(response.data);
            })
            .catch(error => {
    ...
    return (
        <Drawel>
            ...
            <div className="flex justify-between mt-2 mr-8 ml-8 gap-2">
                <div className="mt-7">
                    <Input type="text" name="link" ref={inputRef} value={invoiceDataService?.hashCode} className="w-80 ml-[-14px]" disabled={invoiceDataService?.status === 'inactive'} />
                </div>
                <Button onClick={handleCopy} className="mt-6 mb-6 bg-[#BEC2C6] border border-[#E2E2E2] hover:bg-[#BEC2C6]">Copy</Button>
            </div>
            ...
            {(invoiceDataService?.status === "active") && (
                <div className="mb-2">
                    <div className="mt-2 ml-5 mr-5 flex justify-between">
                        <Button className="bg-white hover:bg-white text-black pr-14 pl-14" onClick={changeStateToInactive}>Deactivate</Button>
                        <Button className="bg-white hover:bg-white text-blue-600 pr-16 pl-16">Edit</Button>
                    </div>
                </div>
            )
            ...
            {(invoiceDataService?.status === "canceled") && (
                <div className="mb-20">
                    <div className="mt-2 ml-5 mr-5 flex justify-between">
                        <Button className="bg-white hover:bg-white text-red pr-14 pl-14" onClick={modalDelete.openModal}>Delete</Button>
                        <Button className="bg-white hover:bg-white text-blue-600 pr-16 pl-16">Edit</Button>
                    </div>
                </div>
            )}
