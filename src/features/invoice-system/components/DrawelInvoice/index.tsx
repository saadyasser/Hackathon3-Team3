...
const DrawelInvoice: any = (): any => {
    ...
    const {
        ...
        error,
        isLoading,
    } = useSwrFetch(API_SERVICES_URLS.CLIENT.INVOICE_DETAIL("641c6d38e524b0786800c225"), {
        method: "GET",
        headers: {},
    });
        ...
        <Drawel>
                ...
                <div className="flex justify-center mt-2">
                    <Card className="w-11/12 bg-[#FFF3F3] border border-[#FFD8D8] shadow-none">
                        <div className="flex gap-3">
                            ...
                            <div>
                                <p className="text-xs font-bold">Invoice Disapproved. Review:</p>
            ...
            <div className="flex justify-center mt-2">
                <div className="w-96">
                    ...
                    {(invoiceData?.status === "pending_approval" || invoiceData?.status === "paid") && (
                        <div className="flex flex-col mr-4 pt-2">
                            <div className="flex justify-end gap-5">
                                <p className="text-[#8C8C8C] text-xs">Subtotal</p>
                                <p className="text-[#8C8C8C] text-xs">${invoiceData?.subTotal}</p>
                            </div>
                ...
                <div className={styles.historyTlContainer}>
                    {timeline?.map((item: any) =>
                        <ul className={styles.tl}>
                            <li className={styles.tlItem} ng-repeat="item in retailer_history">
            ...
            {showInvoice && (invoiceData?.status === "disapproved" || invoiceData?.status === "pending_approval" || invoiceData?.status === "paid" || invoiceData?.status === "sent") && (
                <div className="flex justify-center mt-2 ">
                    <Card className="w-11/12 ">
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between mt-4">
                            <div>
                                ...
                                <p className="text-xs font-bold">{invoiceData?.client?.fullName || "Talents Valley LLC"} </p>
                                <p className="text-xs text-[#797979]">{invoiceData?.client?.address?.country}</p>
                                <p className="text-xs text-[#797979]">{invoiceData?.client?.mobile}</p>
                            </div>
                            <div>
                                <p className="text-xs text-[#797979] mb-2">Bill To</p>
                                <p className="text-xs font-bold">{invoiceData?.freelancer?.firstName} {invoiceData?.freelancer?.lastName}</p>
                                <p className="text-xs text-[#797979] mb-2">{invoiceData?.freelancer.email}</p>
                        ...
                        {fixedInvoice?.map((item: any) =>
                            <div className="flex justify-between mt-4 border-b-2 pb-2">
                                <div>
                                    ...
                                    <p className="text-[#797979] text-xs">{item?.description}</p>
                                </div>
                                <div>
            ...
            {invoiceData?.status === "pending_approval" && (
                <div className="mb-2">
                    ...
                    <div className="mt-2 ml-5 mr-5 flex justify-between mb-10">
                        <Button className="bg-white text-black pr-14 pl-14"onClick={modalCancle.openModal}>Cancle</Button>
            ...
            {(invoiceData?.status === "sent" || invoiceData?.status === "disapproved") && (
                <div className="mb-10">
                    <div className="mt-2 ml-5 mr-5 flex justify-between">
                        <Button className="bg-white text-red pr-14 pl-14" onClick={modalDelete.openModal}>Delete</Button>
                        <Button className="bg-white text-blue-600 pr-16 pl-16">Edit</Button>
                    </div>
                </div>
            )
            ...
            <Modal {...modalDelete} className="!w-80">
                <TransferCard centerTitle={false} closeModal={modalDelete.closeModal}>
                    ...
                    <div className="flex gap-3">
                        <Button fullWidth={true} className="bg-white hover:bg-white text-black border-gray-light border-2" onClick={modalDelete.closeModal}>No</Button>
                        <Button fullWidth={true} className="bg-red hover:bg-red" >Yes</Button>
                    </div>
                </TransferCard>
            </Modal>
            ...
            <Modal {...modalCancle} className="!w-80">
                <TransferCard centerTitle={false} closeModal={modalCancle.closeModal}>
                    ...
                    <div className="flex gap-3">
                        <Button fullWidth={true} className="bg-white hover:bg-white text-black border-gray-light border-2" onClick={modalDelete.closeModal}>Cancle</Button>
